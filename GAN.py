...
#from tensorflow.keras.activations import rrelu as RRELU
from tensorflow_addons.activations import rrelu as RRELU
...
class GAN:
    ...
    def define_encoder_block(self,layer_in,n_filters,batchnorm = True):
        # weight initialization
        init = RandomNormal(stddev = 0.02)
        # add downsampling layer (W-F+2P)/S+1
    ...
    def define_generator(self):
        ...
        bb = Conv2D(512,(4,4),strides = (2,2), padding = 'same',kernel_initializer=init)(e4)
        bb = Activation('relu')(bb)
        # decoder model
        d1 = self.decoder_block(bb,e4,512)
        d2 = self.decoder_block(d1,e3,256)
        # d3 = self.decoder_block(d2,e5,512)
        # d4 = self.decoder_block(d3, e4, 512, dropout = False)
        d3 = self.decoder_block(d2, e2, 128, dropout = False)
        d4 = self.decoder_block(d3, e1, 64, dropout = False)
    ...
    def train(self,d_model,g_model,gan_model,n_epochs=100,n_batch=1):
        ...
        for i in range(n_steps):
            ...
            np.savetxt(self.filename+'/'+'output_new_gan_3.csv', [np.asarray(x_vals),np.asarray(y1_vals),np.asarray(y2_vals),np.asarray(y3_vals)], delimiter=',', fmt='%f')
            # A very essential piece of code to view in run time how my model gets trained
