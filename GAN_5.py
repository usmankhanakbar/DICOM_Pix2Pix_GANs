...
class GAN_5:
    ...
    def define_discriminator(self):
        ...
        #opt = SGD(lr=0.009,momentum=0.9,nesterov=True)
    ...
    def decoder_block(self,layer_in,skip_in,n_filters,dropout = True):
        ...
        in_shape = skip_in.shape
        if in_shape[1]>layer_in.shape[1]:
            g = Conv2D(n_filters,(4,4),strides = (2,2),padding = 'valid',kernel_initializer=init)(layer_in)
        else:
            g = Conv2D(n_filters,(4,4),strides = (2,2),padding = 'same',kernel_initializer=init)(layer_in)
        ...
        g = Concatenate(axis=3)([g, skip_in])
        # relu activation
        g = Activation('relu')(g)
        return g
    ...
    def define_generator(self):
        ...
        # encoder model
        e1 = self.define_encoder_block(in_image,4,batchnorm = False)
        e2 = self.define_encoder_block(e1,8)
        e3 = self.define_encoder_block(e2,16)
        e4 = self.define_encoder_block(e3,32)
        e5 = self.define_encoder_block(e4,64)
        e6 = self.define_encoder_block(e5,128)
        #e7 = self.define_encoder_block(e6,256)
        # bottleneck, no batch norm and relu
        bb = Conv2D(512,(2,2),strides = (2,2), padding = 'same',kernel_initializer=init)(e4)
        bb = Activation('relu')(bb)
        # decoder model
        #d1 = self.decoder_block(bb,e7,256)
        d2 = self.decoder_block(bb,e6,128)
        
        d3 = self.decoder_block(d2, e5, 64)
        d4 = self.decoder_block(d3, e4, 32, dropout = False)
        d5 = self.decoder_block(d4,e3,16,dropout=False)
        d6 = self.decoder_block(d5,e2,8)
        d7 = self.decoder_block(d6,e1,4)
